...
func (u *userRepository) CreateUser(user entity.User) (entity.User, error) {
	...
	log.Infof("Starting to create a new user in the repository layer")
	...
	if err != nil {
		log.Error("Failed to create the user: ", err)
...
func (u *userRepository) GetUserByUsername(username string) (entity.User, error) {
	...
	log := logger.GetLogger()
	log.Info("Starting to retrive a user by username in the repository layer")
	...
	if err != nil {
		log.Error("Failed to retrive the user: ", err)
		return entity.User{}, err
	}

	log.Info("Getting user by username was successfully: ", user)
	return user, nil
}
...
func (u *userRepository) GetUserByID(id string) (entity.User, error) {
	...
	log := logger.GetLogger()
	log.Info("Starting to retrive a user by id in the repository layer")
	...
	log.Info("Getting user by id was successfully: ", user)
	return user, nil

}
func (u *userRepository) UpdateUser(user entity.User) (entity.User, error) {
	log := logger.GetLogger()
	log.Info("Starting to update user in the repository layer")
...
func (u *userRepository) DeleteUser(id string) error {
	log := logger.GetLogger()
	log.Info("Starting to delete user in the repository layer")

	_, err := u.db.Exec(`DELETE FROM mst_user WHERE id_user = $1`, id)

	if err != nil {
		log.Error("Failed to delete the user: ", err)
		return err
	}

	log.Info("User has been deleted successfully: ", id)
	return nil
}
